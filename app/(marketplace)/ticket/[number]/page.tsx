...
import type { ApiResponse, ETicket } from "@/lib/types.gen";
...
export default function TicketPage({ params }: { params: Promise<{ number: string }> }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        // 404 = not paid yet / no ticket issued.
  ...
  return (
    <main className="max-w-xl mx-auto px-4 py-8">
      ...
      <h1 style={{ fontFamily: "var(--font-display)", fontSize: 26, fontWeight: 700, marginBottom: 4 }}>E-tiket</h1>
      ...
      {inv && (
        <div style={card}>
          ...
          <Row label={`Fee Midtrans · ${inv.payment_method_label}`} value={formatRupiah(inv.fee_midtrans)} mono />
