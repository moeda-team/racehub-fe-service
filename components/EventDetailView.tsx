...
export default function EventDetailView({
  ...
  const cta = (
    <Button variant="primary" size="lg" style={{ width: "100%" }} disabled={!interactive || event.quota_remaining <= 0}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 10 }}>
            {categories.map((d) => {
              ...
              return (
                ...
                >
                  <div style={{ fontFamily: "var(--font-display)", fontSize: 19, fontWeight: 700, color: "var(--color-ink)" }}>
                    {d.name}
                  </div>
                  <div style={{ fontSize: 12.5, color: empty ? "var(--color-danger, #C0392B)" : "var(--color-ink-3)", margin: "3px 0 8px" }}>
                    {empty ? "Kuota habis" : `${formatNumber(d.quota_remaining)} dari ${formatNumber(d.quota)} slot tersisa`}
                  </div>
                  <div style={{ height: 4, borderRadius: 2, backgroundColor: "var(--color-line)", overflow: "hidden" }} aria-hidden>
                    <div style={{ height: "100%", width: `${pct * 100}%`, borderRadius: 2, backgroundColor: "var(--color-flame, #F5471D)" }} />
                  </div>
                </div>
              );
            })}
            ...
            {ticketsByDistance.map(({ distance, tickets }) => tickets.length === 0 ? null : (
              <div key={distance.id}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <span style={{ width: 4, height: 16, borderRadius: 2, backgroundColor: "var(--color-flame, #F5471D)" }} aria-hidden />
                  <span style={{ fontFamily: "var(--font-display)", fontSize: 15, fontWeight: 700, color: "var(--color-ink)" }}>
                    {distance.name}
                  </span>
                </div>
                  ...
                  {tickets.map((t) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ fontFamily: "var(--font-mono)", fontWeight: 700, fontSize: 17, color: "var(--color-ink)", whiteSpace: "nowrap" }}>
